...
const loadUsers = async (req, res, next) => {
    ...
    const clientCode = decoded.clientCode;
        ...
        req.brandCollection = tenantDB.collection("brand");
        next();
    } catch (err) {
...
router.get("/", async (req, res) => {
  try {
    const base = await req.brandCollection.find({}).toArray();
    res.json(base);
  } catch (err) {
...
router.post("/", async (req, res) => {
  try {
    const result = await req.brandCollection.insertOne(req.body);
