...
const loadUsers = async (req, res, next) => {
    ...
    const clientCode = decoded.clientCode;
        ...
        req.billsCollection = tenantDB.collection("bills");
        next();
    } catch (err) {
...
router.get("/", async (req, res) => {
  try {
    const base = await req.billsCollection.find({}).toArray();
    res.json(base);
  } catch (err) {
...
router.post("/", async (req, res) => {
  try {
    const result = await req.billsCollection.insertOne(req.body);
